...
`;
function createLawPrioritySelect(sqlLimit) {
    return `
    SELECT
      l.id,
      l.law_key,
...
function mapLawRow(row) {
    const stage = (row.stage ?? row.latest_update_stage ?? "proposed");
    const jurisdictionName = row.jurisdiction_state || row.jurisdiction_country;
    return {
...
function mapLawUpdateRow(row) {
    const stage = (row.stage ?? "proposed");
    return {
        id: row.id,
...
function createApp(db) {
    ...
    app.get("/api/brief", (req, res) => {
        ...
        const rows = db.prepare(createLawPrioritySelect(limit)).all();
        const lastCrawl = (0, db_1.getLatestCrawlRun)(db);
    ...
    app.get("/api/laws/:lawKey", (req, res) => {
    ...
    app.get("/api/laws/:lawKey/updates", (req, res) => {
        const lawKey = decodeURIComponent(String(req.params.lawKey));
        const law = db.prepare("SELECT id FROM laws WHERE law_key = ?").get(lawKey);
        if (!law) {
            return res.status(404).json({ error: "law not found" });
        }
        const updates = db
    ...
    app.post("/api/laws/rebuild", (_req, res) => {
        const stats = (0, db_1.backfillLawsFromEvents)(db);
