...
function extractLawPhrase(text) {
    const normalized = normalizeWhitespace(text);
    for (const acronym of knownLawAcronyms) {
        const regex = new RegExp(`\\b${acronym}\\b`, "i");
        if (regex.test(normalized)) {
            const companion = new RegExp(`([A-Z][A-Za-z0-9'’(),\\- ]{2,120}(?:${lawKeywords.join("|")}))`).exec(normalized);
    ...
    if (billNumberMatch) {
        ...
        const prefix = /([A-Z][A-Za-z0-9'’(),\- ]{2,80}\b(?:Act|Bill|Directive|Regulation|Code|Rule)\b)/i.exec(normalized)?.[1]
            ?? `${identifier} Bill`;
        return {
...
function fallbackLawName(input) {
    ...
    const legalPhrase = /\b([A-Za-z][A-Za-z0-9'’\- ]{4,80}(?:privacy|children|youth|minor|online safety|digital safety|data protection|consent|age verification))/i.exec(combined)?.[1];
    if (legalPhrase) {
...
function inferCanonicalLaw(input) {
    const parsed = extractLawPhrase(`${input.title} ${input.summary ?? ""} ${input.content ?? ""}`)
