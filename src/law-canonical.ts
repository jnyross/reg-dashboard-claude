...
function toLawTitleCase(value: string): string {
...
function resolveLawType(name: string): string {
  const keyword = lawKeywords.find((candidate) => new RegExp(`\\b${candidate}\\b`, "i").test(name));
  return keyword ? keyword.toLowerCase() : "law";
}

function extractKnownLawAlias(text: string, jurisdictionCountry: string): ParsedLaw | null {
...
function scoreCandidateLawName(name: string): number {
  const words = name.split(/\s+/).filter(Boolean);
  let score = 0;

  if (keywordPattern.test(name)) score += 10;
  if (/\b\d{4}\b/.test(name)) score += 2;
  if (/\b(?:COPPA|KOSA|DSA|GDPR|DPDP|PDPA|AB-\d+)\b/.test(name)) score += 3;
  if (narrativePrefixPattern.test(name) || narrativeVerbPattern.test(name)) score -= 8;
  score -= Math.max(0, words.length - 9);

  return score;
}

function extractExplicitLawPhrase(text: string): ParsedLaw | null {
  const normalized = normalizeWhitespace(text)
    .replace(/[“”]/g, '"')
    .replace(/[‘’]/g, "'");

  const candidates: ParsedLaw[] = [];
  ...
  for (const match of matches) {
    ...
    if (head.length === 0 || head.length > 8) continue;
    if (narrativeVerbPattern.test(head.join(" "))) continue;
...
function fallbackLawName(input: CanonicalLawInput): ParsedLaw {
  ...
  const conciseTitle = toLawTitleCase(
    title
      .replace(/^[^A-Za-z0-9]+/, "")
      .replace(/[.!?].*$/, "")
      .split(" ")
...
export function inferCanonicalLaw(input: CanonicalLawInput): CanonicalLaw {
  ...
  let parsed: ParsedLaw | null = null;
