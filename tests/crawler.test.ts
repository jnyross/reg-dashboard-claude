import { crawlSource, crawlAllSources, type CrawledItem } from "../src/crawler";
...
describe("crawlSource", () => {
  it("crawls a government page and extracts title + text", async () => {
    ...
    const items = await crawlSource(testSource);
  ...
  it("parses RSS feed items", async () => {
    const rssSource: RegistrySource = {
      ...testSource,
      name: "Test RSS Feed",
      type: "rss_feed",
    };
    ...
    const items = await crawlSource(rssSource);
...
describe("crawlAllSources", () => {
  it("crawls multiple sources with concurrency", async () => {
    ...
    const items = await crawlAllSources(sources, 2);
  ...
  it("reports progress", async () => {
    ...
    await crawlAllSources(sources, 5, (completed, total, name) => {
  ...
  it("continues on individual source failures", async () => {
    ...
    let callCount = 0;
    global.fetch = jest.fn(async (url: string | URL | globalThis.Request) => {
      callCount++;
    ...
    const items = await crawlAllSources(sources, 5);
