...
import { initializeSchema, openDatabase, getLatestCrawlRun } from "../src/db";
...
describe("GET /api/events with ageBracket filter", () => {
  it("accepts valid ageBracket filter values", async () => {
    ...
    const r1 = await request(app).get("/api/events?ageBracket=13-15");
    expect(r1.status).toBe(200);

    const r2 = await request(app).get("/api/events?ageBracket=16-18");
    expect(r2.status).toBe(200);

    const r3 = await request(app).get("/api/events?ageBracket=both");
  ...
  it("rejects invalid ageBracket values", async () => {
    const { app, db } = buildTestApp();
    const response = await request(app).get("/api/events?ageBracket=invalid");
